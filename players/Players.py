...
class Player:
	...
	def add_resource(self, resource: str):
		if len(resource)>1:
			raise Exception(f'The lengh of the resource is too long: len(ressources) = {len(resource)}')
		self.resources.append(resource)

	def add_half_resources(self, half_resource: str):
		if len(half_ressources)>3:
			return Exception(f'The lengh of the half ressource is too long: len(half_ressource) = {half_resources}') 
		self.half_resources.append((half_resource[0], half_resource[2]))
	...
	def all_resources_combination_possible(self, with_yellow = True) -> list[list]:
		...
		for choice in choice_resources_player:
			for i in range(len(choice)):
				for j in range(size_of_vector/len(choice)* i, size_of_vector/len(choice)*(i+1))
